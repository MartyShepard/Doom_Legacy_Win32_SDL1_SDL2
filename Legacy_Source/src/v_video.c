...
consvar_t * video_cvar_list[] =
{
  ...
  &cv_gammafunc,
    ...
    // Screen
  &cv_fullscreen,     // only for opengl so use differant name please and move it to differant place
  &cv_scr_depth,
  &cv_scr_width,
  &cv_scr_height,
  &cv_fuzzymode,
...
void V_Init_VideoControl( void )
{
    ...
    CV_RegisterVar_list( video_cvar_list );
}
...
void V_SetupDraw( uint32_t screenflags )
{
    ...
    drawinfo.x_unitfrac = FixedDiv(FRACUNIT, INT_TO_FIXED(drawinfo.dupx));
    drawinfo.y_unitfrac = FixedDiv(FRACUNIT, INT_TO_FIXED(drawinfo.dupy));
...
//    destend = desttop + (patch->width * drawinfo.xbytes);  // test against desttop
    ...
    wf = INT_TO_FIXED( patch->width );

    for (col=0; col < wf; col += drawinfo.x_unitfrac)
    {
        column = (column_t *) ((byte *) patch + patch->columnofs[ FIXED_TO_INT(col) ]);
        ...
        while (column->topdelta != 0xff)
        {
            ...
            if(vid.drawmode != DRAW8PAL)
            {
                while (count--)
                {
                    V_DrawPixel( dest, 0, colormap[ source[ FIXED_TO_INT(ofs) ]] );
                    dest += vid.ybytes;
...
#endif
            {
                ...
                while (count--)
                { 
                    *dest = colormap[ source[ FIXED_TO_INT(ofs) ]];
                    dest += vid.ybytes;
                    ofs += drawinfo.y_unitfrac;
                }
            }
...
void V_DrawScaledPatch(int x, int y, patch_t * patch)
{
    ...
    {
        colfrac = -colfrac;
        col = INT_TO_FIXED(patch->width) + colfrac;
    }
    else
        col = 0;
            ...
            if(vid.drawmode != DRAW8PAL)
            {
                while (count--)
                {
                    ...
                    if( dest >= drawinfo.screen_start )
                       V_DrawPixel( dest, 0, source[ FIXED_TO_INT(ofs) ] );
#else
                    V_DrawPixel( dest, 0, source[ FIXED_TO_INT(ofs) ] );
#endif
                    dest += vid.ybytes;
...
//    boolean skippixels = false;
    ...
    colfrac = FixedDiv(FRACUNIT, INT_TO_FIXED(dupx));
    rowfrac = FixedDiv(FRACUNIT, INT_TO_FIXED(dupy));
        ...
        while (column->topdelta != 0xff)
        {
            ...
            switch(vid.drawmode)
            {
             ...
             case DRAW8PAL:
                while (count--)
                {
                    register unsigned int color = source[ FIXED_TO_INT(ofs) ];
...
//        dest = screens[0] + (y * vid.ybytes) + (x * vid.bytepp);
    ...
    dx = FixedDiv( FRACUNIT,
         ((((vid.dupx-1) << (FRACBITS-4)) * scale) + FIXINT(1)) );
    dy = FixedDiv( FRACUNIT,
         ((((vid.dupy-1) << (FRACBITS-4)) * scale) + FIXINT(1)) );
    ...
    for (v = 0; v < h; v++)
    {
        ...
        for (u = 0; u < w; u++)
        {
            V_DrawPixel(dest, u, src[ FIXED_TO_INT(xfrac) & imask ]);
            xfrac += dx;
        }
        yfrac += dy;
        dest += vid.ybytes;
...
void V_DrawTiltView(byte * viewbuffer)
{
    ...
    leftxfrac = INT_TO_FIXED(vertex[0].px);
    leftyfrac = INT_TO_FIXED(vertex[0].py);

    // steps
    xstep = INT_TO_FIXED(vertex[3].px - vertex[0].px) / vid.height;
    ystep = INT_TO_FIXED(vertex[3].py - vertex[0].py) / vid.height;
    ...
    ds_x1 = 0;
    ds_x2 = vid.width - 1;
    ds_xstep = INT_TO_FIXED(vertex[1].px - vertex[0].px) / vid.width;
    ds_ystep = INT_TO_FIXED(vertex[1].py - vertex[0].py) / vid.width;
...
void V_DrawPerspView(byte * viewbuffer, int aiming)
{
    ...
    // vid.direct not allowed without locking the video buffer
        ...
        xfrac = FIXINT(20) + ((!x1) & 0xFFFF);
        xfracstep = FixedDiv( INT_TO_FIXED(vid.width) - (xfrac << 1), scale);
        w = scale >> 16;
#ifdef ENABLE_DRAWEXT
        if( vid.bytepp > 1 )
        {
          while (w--)
          {
            V_DrawPixel( dest, 0, source[ FIXED_TO_INT(xfrac) ] );
            dest += vid.bytepp;
            xfrac += xfracstep;
          }
        }
...
#endif
        {
          ...
          while (w--)
          {
            *dest++ = source[ FIXED_TO_INT(xfrac) ];
            xfrac += xfracstep;
          }
        }
