...
boolean cht_Responder(event_t * ev)
{
...
#ifdef SDL2
    // [WDJ] Ignore the translated text of ev_textchar here,
    // as the difference is not worth the problems.
#endif
    player_t* plyr;
    ...
    if (ev->type == ev_keydown)
    {
        ...
        if (cht_CheckCheat(&cheat_clev, ch))
        {
            ...
            if ((gamemode == GM_ultdoom_retail) && ((epsd > 4) || (map > 9)))
                return false;

            if ((gamemode == GM_doom_registered) && ((epsd > 3) || (map > 9)))
                return false;

            if ((gamemode == GM_doom_shareware) && ((epsd > 1) || (map > 9)))
                return false;

            if ((gamemode == GM_doom2_commercial) && ((epsd > 1) || (map > 34)))
                return false;
...
void Command_CheatGimme_f(void)
{
    ...
    for (k = 0; k < 2; k++)
    {
      ...
      for (i = 1; i < COM_Argc(); i++)
      {
        ...
        {
            if (gamemode == GM_doom2_commercial) // only in Doom2
            {
