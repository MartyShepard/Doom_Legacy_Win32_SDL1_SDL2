...
// T_FireFlicker
//
// thinker_function[ TFI_FireFlicker ] (fireflicker_t* flick)
...
// Do flashing lights.
//
// thinker_function[ TFI_LightFlash ] (lightflash_t* flash)
...
// T_StrobeFlash
//
// thinker_function[ TFI_StrobeFlash ] (strobe_t*  flash)
...
// Spawn glowing light
//
// thinker_function[ TFI_Glow ] (glow_t* gp)
...
//
// thinker_function[ TFI_LightFade ] (lightfader_t * lf)
...
//
//   line :  has sector TAG
//   level : light level fraction, 0=min, FIXINT(1)=max, else interpolate min..max
// Returns true
int EV_LightTurnOnPartway(line_t * line, fixed_t level)
{
  ...
  while ( (i = P_FindSectorFromLineTag(line,i)) >= 0 )
  {
      ...
      sector->lightlevel = FIXED_TO_INT(level * maxll + (FRACUNIT - level) * minll);
  }
  return 1;
}
