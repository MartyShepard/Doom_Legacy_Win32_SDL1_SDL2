...
static void WI_Init_AnimatedBack(void)
{
    ...
    if (gamemode == GM_doom2_commercial || gamemode == GM_heretic || *info_interpic)
        return;
...
static void WI_Draw_ShowNextLoc(void)
{
    ...
    if (EN_doom_etc)
    {
        uint8_t draw_el = ! ((gamemode == GM_doom2_commercial) && (wbs->lev_next == 30));  // any map but map31
...
#else
    if ( EN_doom_etc
         && !((gamemode == GM_doom2_commercial) && (wbs->lev_next == 30)) )   // any map but map31
        WI_Draw_EL();
#endif
}
...
void WI_Release_Data(void)
{
    ...
    if( lnames )
    {
      ...
      if( (gamemode != GM_doom2_commercial) && wbs )
      {
        wb_epsd = wbs->epsd;
...
// printf( "WI RANGECHECK: episode=%i, lev_prev=%i, lev_next=%i\n", wbs->epsd, wbs->lev_prev, wbs->lev_next );
    ...
    else
    {
        ...
        episode_max = ( gamemode == GM_ultdoom_retail )? 3 : 2;  // 0..3, as 0 based index
# ifdef ENABLE_UMAPINFO
