...
manual_floor:                
    ...
    switch(Targ)
    {
      ...
      case FTAR_FbyST:
        {
          int fdh = FIXED_TO_INT(mfloor->sector->floorheight) +
                    (mfloor->direction * FIXED_TO_INT(P_FindShortestTextureAround(secnum)));
          if(fdh > 32000)   fdh = 32000;
          if(fdh < -32000)  fdh = -32000;
          mfloor->floordestheight = INT_TO_FIXED(fdh);
        }
        break;
      case FTAR_Fby24:
        mfloor->floordestheight = mfloor->sector->floorheight +
          mfloor->direction * FIXINT(24);
        break;
      case FTAR_Fby32:
        mfloor->floordestheight = mfloor->sector->floorheight +
          mfloor->direction * FIXINT(32);
        break;
      default:
        break;
    }
...
manual_ceiling:                
    ...
    ceiling->thinker.function = TFI_MoveCeiling;
    ceiling->crush = Crsh;
    ...
    switch(Targ)
    {
      ...
      case CTAR_CbyST:
        {
          int th = FIXED_TO_INT(ceiling->sector->ceilingheight) +
                   (ceiling->direction * FIXED_TO_INT(P_FindShortestUpperAround(secnum)));
          if(th > 32000)  th = 32000;
          if(th < -32000) th = -32000;
          targheight = INT_TO_FIXED(th);
        }
        break;
      case CTAR_Cby24:
        targheight = ceiling->sector->ceilingheight +
          ceiling->direction * FIXINT(24);
        break;
      case CTAR_Cby32:
        targheight = ceiling->sector->ceilingheight +
          ceiling->direction * FIXINT(32);
        break;
      default:
        break;
    }
...
manual_lift:
    ...
    plat->thinker.function = TFI_PlatRaise;
    plat->crush = false;
    plat->tag = line->tag;
    ...
    // [WDJ] 1/15/2009 Add DOORDELAY_CONTROL of delay_ticks_per_sec.
    // setup the delay time before the floor returns
    switch(Dely)
    {
      case 0:
        plat->wait = 1 * delay_ticks_per_sec;  // [WDJ]
        break;
      case 1:
        plat->wait = PLATWAIT * delay_ticks_per_sec;  // [WDJ]
        break;
      case 2:
        plat->wait = 5 * delay_ticks_per_sec;  // [WDJ]
        break;
      case 3:
        plat->wait = 10 * delay_ticks_per_sec;  // [WDJ]
        break;
    }
...
manual_stair:          
    ...
    do
    {
      ...
      for (i = 0; i < sec->linecount; i++)
      {
        ...
        if (!Igno && tsec->floorpic != adj_texture)
          continue;
...
manual_crusher:                
    ...
    ceiling->bottomheight = sec->floorheight + FIXINT(8);
...
int EV_DoGenLockedDoor ( line_t* line )
{
  ...
  unsigned int  value = line_special - GenLockedBase;
...
manual_locked:
    ...
    door->lighttag =
        ( EN_doorlight
          && (line_special&6) == 6
          && line_special > GenLockedBase ) ? line->tag : 0;

#ifdef DEBUG_LIGHTTAG
    if( door->lighttag )
    {
        // Detect fault situation in cyberarena3 map02.
        // Cannot reproduce this, so have to detect the initiation.
        printf( "DoGenLockedDoor: Lighttag = %i, line# = %i\n", door->lighttag, line - lines );
    }
#endif
...
manual_door:
    ...
    if( door->lighttag )
    {
        // Detect fault situation in cyberarena3 map02.
        // Cannot reproduce this, so have to detect the initiation.
        printf( "DoGenDoor: Lighttag = %i, line# = %i\n", door->lighttag, line - lines );
    }
#endif
