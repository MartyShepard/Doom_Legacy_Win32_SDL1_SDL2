...
void P_AddMaceSpot(mapthing_t* mthing)
{
    ...
    MaceSpots[MaceSpotCount].x = INT_TO_FIXED(mthing->x);
    MaceSpots[MaceSpotCount].y = INT_TO_FIXED(mthing->y);
    MaceSpotCount++;
}
...
//----------------------------------------------------------------------------
//
// PROC T_BlasterMobjThinker
//
// Thinker for the ultra-fast blaster PL2 ripper-spawning missile.
//
//----------------------------------------------------------------------------

// thinker_function[ TFI_BlasterMobjThinker ] (mobj_t* mobj)
void T_BlasterMobjThinker(mobj_t* mobj)
{
    int i;
...
void A_FireBlasterPL2(player_t* player, pspdef_t* psp)
{
    ...
    if( mo )
      mo->thinker.function = TFI_BlasterMobjThinker;
...
void A_FireMacePL1B(player_t* player, pspdef_t* psp)
{
    ...
    ball = P_SpawnMobj(pmo->x, pmo->y,
                       pmo->z + FIXINT(28) - FOOTCLIPSIZE*((pmo->flags2&MF2_FEETARECLIPPED) != 0),
                       MT_MACEFX2);
    ball->momz = FIXINT(2) + ((player->aiming)<<(FRACBITS-5));
...
void A_FireMacePL1(player_t* player, pspdef_t* psp)
{
    ...
    player->ammo[am_mace] -= USE_MACE_AMMO_1;
    psp->sx = INT_TO_FIXED((PP_Random(ph_maceatk1)&3) - 2);
    psp->sy = WEAPONTOP + INT_TO_FIXED(PP_Random(ph_maceatk1)&3);
...
void A_MacePL1Check(mobj_t * ball)
{
    ...
    ball->momx = FixedMul( FIXINT(7), finecosine[angf]);
    ball->momy = FixedMul( FIXINT(7), finesine[angf]);
...
void A_MaceBallImpact2(mobj_t * ball)
{
    ...
    if((ball->z != ball->floorz) || (ball->momz < FIXINT(2)))
    { // Explode
    ...
    { // Bounce
        ball->momz = (ball->momz*192)>>8;
        zm = ball->momz - FIXINT(1);  // momz of FX3
        P_SetMobjState(ball, ball->info->spawnstate);
...
void A_FireMacePL2(player_t* player, pspdef_t* psp)
{
    ...
    if(mo)
    {
        mo->momx += pmo->momx;
        mo->momy += pmo->momy;
        mo->momz = FIXINT(2) + ((player->aiming)<<(FRACBITS-5));
...
void A_DeathBallImpact(mobj_t* ball)
{
    ...
    { // Bounce
        ...
        { // Find new target
            for(i = 0; i < 16; i++)
            {
                P_AimLineAttack(ball, angle, FIXINT(10 * 64), 0);
        ...
        if(newAngle)
        {
            ...
            int angf = ANGLE_TO_FINE(angle);
#ifdef MONSTER_VARY
            ball->momx = FixedMul(ball->speed, finecosine[angf]);
            ball->momy = FixedMul(ball->speed, finesine[angf]);
#else
...
void A_SpawnRippers(mobj_t* actor)
{
    ...
    for(i = 0; i < 8; i++)
    {
        ...
        int angf = ANGLE_TO_FINE(angle);
#ifdef MONSTER_VARY
        ripper->momx = FixedMul(ripper->speed, finecosine[angf]);
        ripper->momy = FixedMul(ripper->speed, finesine[angf]);
#else
...
void A_SkullRodStorm(mobj_t* actor)
{
    ...
    x = actor->x + INT_TO_FIXED((PP_Random(ph_skullrodrain)&127) - 64);
    y = actor->y + INT_TO_FIXED((PP_Random(ph_skullrodrain)&127) - 64);
...
{
    actor->z = actor->ceilingz + FIXINT(4);
}
...
void A_FirePhoenixPL1(player_t* player, pspdef_t* psp)
{
    ...
    pmo->momx += FixedMul( FIXINT(4), finecosine[angf]);
    pmo->momy += FixedMul( FIXINT(4), finesine[angf]);
}
...
void A_PhoenixPuff(mobj_t* actor)
{
    ...
    puff->momx = FixedMul( FIXFL(1.3), finecosine[angf]);
    puff->momy = FixedMul( FIXFL(1.3), finesine[angf]);
    puff->momz = 0;
...
void A_FirePhoenixPL2(player_t* player, pspdef_t* psp)
{
    ...
    z = pmo->z + FIXINT(26) + INT_TO_FIXED(player->aiming)/173;
    ...
    int angf = ANGLE_TO_FINE(angle);
#ifdef MONSTER_VARY
    mo->momx = pmo->momx + FixedMul(mo->speed, finecosine[angf]);
    mo->momy = pmo->momy + FixedMul(mo->speed, finesine[angf]);
#else
...
void A_GauntletAttack(player_t* player, pspdef_t* psp)
{
    ...
    psp->sx = INT_TO_FIXED((PP_Random(ph_gauntlet)&3) - 2);
    psp->sy = WEAPONTOP + INT_TO_FIXED(PP_Random(ph_gauntlet)&3);
    angle = pmo->angle;
    if(player->powers[pw_weaponlevel2])
    {
        damage = HITDICE(2);
