                     ...
                     int        flags)
{
    ...
    fracsplat = FixedDiv( (INT_TO_FIXED(patch->width)>>1) , linelength );
...
//    debug_Printf("added a floor splat\n");
    ...
    //
    splat->z = z;
    splat->verts[0].x = splat->verts[3].x = x - FIXINT(32);
    splat->verts[2].x = splat->verts[1].x = x + FIXINT(31);
    splat->verts[3].y = splat->verts[2].y = y + FIXINT(31);
    splat->verts[0].y = splat->verts[1].y = y - FIXINT(32);
...
static void R_RenderFloorSplat (floorsplat_t* pSplat, vertex_t* verts, byte* pTex)
{
    ...
    for (y=miny; y<=maxy; y++)
    {
        x1 = FIXED_TO_INT(rastertab[y].minx);
        x2 = FIXED_TO_INT(rastertab[y].maxx);
...
void R_DrawVisibleFloorSplats (void)
{
    ...
    while (pSplat)
    {
        ...
        for (i=0; i<4; i++)
        {
            ...
            // projection
            v2d[i].x = FIXED_TO_INT(centerxfrac + FixedMul(rot_x, scale_x));
            v2d[i].y = FIXED_TO_INT(centeryfrac + FixedMul(rot_z, scale_y));
        }
        /*
