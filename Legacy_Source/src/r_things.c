...
int R_AddSingleSpriteDef (char* sprname, spritedef_t* spritedef, int wadnum, int startlump, int endlump)
{
    ...
    for (l=startlump ; l<endlump ; l++)
    {
        ...
        if( NAME4_AS_NUM(lumpname) == numname )
        {
            ...
            sl->width = INT_TO_FIXED((uint32_t)((uint16_t)( LE_SWAP16(patch.width) ))); // unsigned
            sl->leftoffset = INT_TO_FIXED((int32_t)LE_SWAP16(patch.leftoffset));  // signed
            sl->topoffset = INT_TO_FIXED((int32_t)LE_SWAP16(patch.topoffset));  // signed
            sl->height = INT_TO_FIXED((uint32_t)((uint16_t)( LE_SWAP16(patch.height) )));  // unsigned
...
// This does not adjust to changes in drawmode, leaving objects embedded in floor.
            ...
            if( rendermode != render_soft )
            {
                ...
                if( p_topoffset>0 && p_topoffset<p_height) // not for psprite
                {
                    ...
                    sl->topoffset =
                       INT_TO_FIXED( min(p_topoffset+4, p_height) );
                }
            }
#endif
#endif
...
// It needs to position using scale, and draw patch using yscale.
        ...
        dc_yl = FIXED_TO_INT( top_post_sc + FRACUNIT-1 );
        dc_yh = FIXED_TO_INT( bottom_post_sc - 1 );

        if(dm_windowtop != FIXED_MAX && dm_windowbottom != FIXED_MAX)
        {
          // screen coord. where +y is down screen
          if(dm_windowtop > top_post_sc)
            dc_yl = FIXED_TO_INT( dm_windowtop + FRACUNIT-1 );
          if(dm_windowbottom < bottom_post_sc)
            dc_yh = FIXED_TO_INT( dm_windowbottom - 1 );
        }
    ...
    if ( dc_yh >= rdraw_viewheight )
    {
        ...
        dc_yh = rdraw_viewheight - 1;
            ...
            dc_texturemid = dm_texturemid - INT_TO_FIXED( cur_topdelta );
            // dc_source = (byte *)column + 3 - cur_topdelta;
#else
            // Normal patch	     
            dc_texturemid = dm_texturemid - INT_TO_FIXED( column->topdelta );
...
//        colfunc = fuzzcolfunc;
    ...
    if(vis->scale != vis->yscale)  // monster_vary is in effect
    {
        ...
        dc_y0 = FIXED_TO_INT( dm_top_patch );
    ...
    {
        texturecolumn = FIXED_TO_INT( texcol_frac );
#ifdef RANGECHECK
...
void R_Split_Sprite_over_FFloor (vissprite_t* sprite, mobj_t* thing)
{
  ...
  for( gi = 1; gi < sector->numlights; gi++)	// from top to bottom
  {
    ...
    cut_y = FIXED_TO_INT( centeryfrac - FixedMul(lightheight - viewz, sprite->scale) );
    if(cut_y < 0)
            continue;
...
static void R_ProjectSprite (mobj_t* thing)
{
    ...
    x1 = FIXED_TO_INT( centerxfrac + FixedMul(tx, xscale) );
...
#ifdef MONSTER_VARY
    x2 = FIXED_TO_INT( centerxfrac + FixedMul(tx + draw_width, xscale) ) - 1;
#else
    x2 = FIXED_TO_INT( centerxfrac + FixedMul(tx + sprlump->width, xscale) ) - 1;
#endif
 ...
 //   vis->mobj_height = thing->height;  // unused
    ...
    vis->y_top = FIXED_TO_INT( centeryfrac - FixedMul(vis->gz_top - viewz, yscale) );
    vis->y_bot = FIXED_TO_INT( centeryfrac - FixedMul(vis->gz_bot - viewz, yscale) );
...
//    sprframe = get_spriteframe( sprdef, fr );
    ...
    tx = psp->sx - FIXINT(BASEVIDWIDTH/2);
    ...
    tx -= sprlump->leftoffset;
#endif
    x1 = FIXED_TO_INT( centerxfrac + FixedMul(tx, pspritescale) );
    ...
    tx += sprlump->width;
    x2 = FIXED_TO_INT( centerxfrac + FixedMul(tx, pspritescale) ) - 1;
    ...
    vis->texturemid = (cv_splitscreen.EV) ?
        FIXINT(120) + FRACUNIT/2 - (psp->sy - sprlump->topoffset)
        : FIXINT(BASEYCENTER) + FRACUNIT/2 - (psp->sy - sprlump->topoffset);
    ...
    if(viewer_sector->numlights)
    {
      ...
      ff_light_t * ff_light =
        R_GetPlaneLight(viewer_sector, viewmobj->z + FIXINT(41));
...
void R_DrawPlayerSprites (void)
{
    ...
    centeryfrac = INT_TO_FIXED( centery );  //for R_DrawVisSprite
...
setup_corona :
    ...
    corona_sprlump.width = INT_TO_FIXED( corona_patch->width );
    corona_sprlump.height = INT_TO_FIXED( corona_patch->height );
    corona_sprlump.leftoffset = INT_TO_FIXED( corona_patch->leftoffset );
    corona_sprlump.topoffset = INT_TO_FIXED( corona_patch->topoffset );
    return;
}
...
# endif
    corona_x0 = corona_x1 = FIXED_TO_INT( midx - FixedMul(corona_sprlump.leftoffset, corona_xscale));
    corona_x2 = FIXED_TO_INT( midx + FixedMul(corona_sprlump.width - corona_sprlump.leftoffset, corona_xscale) ) - 1;
...
//    dc_colormap = & reg_colormaps[0];
    ...
    fixed_t light_yoffset = FLOAT_TO_FIXED( corona_lsp->light_yoffset );
