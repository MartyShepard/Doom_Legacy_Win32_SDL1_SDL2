...
gamemode_e  gamemode = GM_indetermined;   // Game Mode - identify IWAD as shareware, retail etc.
...
consvar_t cv_tired_run        = {"tiredrun" ,"0",CV_SAVE | CV_NETVAR, tired_cons_t};
consvar_t cv_drown            = {"drown"    ,"0",CV_SAVE | CV_NETVAR, drown_cons_t};
#endif
...
byte NextWeapon( player_t * player, int step )
{
    ...
    for ( ; nextweaponorder[i] != plwpn; i = (i+NUM_WEAPONS+step)%NUM_WEAPONS )
    {
        ...
        if( gamemode != GM_doom2_commercial && (w == wp_supershotgun))
            continue;

        // skip plasma-bfg in shareware
        if( gamemode == GM_doom_shareware && (w==wp_plasma || w==wp_bfg))
            continue;
...
void G_DoLoadLevel (boolean resetplayer)
{
    ...
    fs_fadealpha = 0;
    extramovefactor = 0;
    jumpgravity = (FIXINT(6)/NEWTICRATERATIO);  // re-init
...
boolean  G_SpawnExtraDog( mapthing_t * spot )
{
    ...
    // Don't have a valid doomed number, spawn by MT_xxx code.
    P_Spawn_Mapthing_MT( &extra_dog_spot, extra_dog_MT );
...
beyond_std_setup:
    ...
    else if ( gamemode == GM_doom2_commercial )
...
boolean G_Downgrade(int version)
{
    ...
    {
        friction_model =
           (gamemode == GM_heretic)? FR_heretic
         : (gamemode == GM_hexen)? FR_hexen
