...
void DemoAdapt_p_floor( void )
{
    ...
    // [WDJ] DSDA has a mbf compatibility level that blocks this fix.
    // MBF did not have the stair build fix.
}
...
//
// thinker_function[ TFI_MoveFloor ] (floormove_t* mfloor)
void T_MoveFloor(floormove_t* mfloor)
{
    ...
    if (res == MP_pastdest)
    {
        ...
        if ((mfloor->type == FT_buildStair && gamemode == GM_heretic) || 
            gamemode != GM_heretic)
...
//
// thinker_function[ TFI_MoveElevator ] (elevator_t* elevator)
...
//            mfloor->sector = sec;
            mfloor->speed = FLOORSPEED;
            mfloor->floordestheight = mfloor->sector->floorheight + FIXINT(24);
            break;
...
//            mfloor->sector = sec;
            ...
            mfloor->floordestheight = mfloor->sector->floorheight + FIXINT(32);
            break;
...
//            mfloor->sector = sec;
            mfloor->speed = FLOORSPEED * 4;
            mfloor->floordestheight = P_FindHighestFloorSurrounding(sec);
            if (mfloor->floordestheight != sec->floorheight || gamemode == GM_heretic )
                mfloor->floordestheight += FIXINT(8);
            break;
...
//            mfloor->sector = sec;
            ...
            mfloor->floordestheight -= FIXINT(8) * (floortype == FT_raiseFloorCrush);
            break;
...
//            mfloor->sector = sec;
            mfloor->speed = FLOORSPEED;
            mfloor->floordestheight = mfloor->sector->floorheight + FIXINT(512);
            break;
...
int EV_BuildStairs ( line_t*  line, stair_e type )
{
  ...
  fixed_t               stair_height;
  fixed_t               stairsize;
  fixed_t               speed;

  // [WDJ] crushing, speed, stairsize, are constant for the loop.
  // Set up the speed and stepsize according to the stairs type.
  // [WDJ] PrBoom had compatibility with bug due to uninitialized crush field.
  // DoomLegacy does not emulate this bug.
  switch(type)
  {
    case ST_build8:
      speed = FLOORSPEED/4;
  ...
  while ((secnum = P_FindSectorFromLineTag(line,secnum)) >= 0)
  {
    ...
    mfloor->speed = speed;
    stair_height = sec->floorheight + stairsize;
    mfloor->floordestheight = stair_height;
