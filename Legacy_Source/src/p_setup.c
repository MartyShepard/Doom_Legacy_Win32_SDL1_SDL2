...
static
void P_GetNodesVersion( lumpnum_t lumpnum, lumpnum_t gl_lumpnum )
{
  const void * data;
...
void P_LoadVertexes (lumpnum_t lumpnum)
{
    ...
    {
        // signed
        li->x = INT_TO_FIXED( LE_SWAP16(ml->x) );
        li->y = INT_TO_FIXED( LE_SWAP16(ml->y) );
    }
...
// Return bytes of memory used for flats.
unsigned int P_PrecacheLevelFlats( void )
{
  unsigned int flatmemory = 0;
  unsigned int i;
  lumpnum_t lump;
...
uint16_t P_AddLevelFlat( const char* flatname )
{
    lump_name_t name8;
    levelflat_t * lfp;
    ...
    if( numlevelflats > 0xFFF7 )
        I_Error("number of flats exceeds id size\n" );
...
//  id : index in levelflats
char * P_FlatNameForNum(uint16_t id)
{
  if(id < 0 || id > numlevelflats)
    I_Error("P_FlatNameForNum: Invalid flatnum\n");

  return Z_Strdup(va("%.8s", levelflats[id].name), PU_STATIC, 0);
}
...
void P_LoadSectors( lumpnum_t lumpnum )
{
    ...
    {
        // signed
        ss->floorheight = INT_TO_FIXED( LE_SWAP16(ms->floorheight) );
        ss->ceilingheight = INT_TO_FIXED( LE_SWAP16(ms->ceilingheight) );
...
void P_LoadSideDefs (int lump)
{
    ...
    {
        sd->textureoffset = INT_TO_FIXED( LE_SWAP16(msd->textureoffset) );
        sd->rowoffset = INT_TO_FIXED( LE_SWAP16(msd->rowoffset) );
...
void P_LoadSideDefs2(int lump)
{
  ...
  {
      mapsidedef_t * msd = & msdlump[sdnum]; // map sidedef
      side_t * sd = & sides[sdnum];
      sector_t * sec;
...
void P_LoadBlockMap (int lump)
{
  ...
  bmaporgx = INT_TO_FIXED( blockmaphead[0] );
  bmaporgy = INT_TO_FIXED( blockmaphead[1] );
...
boolean  P_CheckHeaderLumps( lumpnum_t lump )
{
  ...
  unsigned int filen = WADFILENUM(lump);
...
void P_Setup_LevelSky (void)
{
    ...
    if (gamemode == GM_doom2_commercial)  // includes doom2, plut, tnt
      // || (gamemode == GM_pack_tnt) he ! is not a mode is a episode !
      //    || ( gamemode == GM_pack_plut )
    {
        if (gamemap < 12)
            sn = "SKY1";
        else
...
//    R_Flush_Texture_Cache();
    ...
    P_Init_Monsters ();  // Heretic
    P_OpenWeapons ();  // Heretic

    P_LoadThings (level_lumpnum+ML_THINGS);

    P_CloseWeapons ();  // Heretic
...
boolean P_AddWadFile (char* wadfilename, /*OUT*/ level_id_t * firstmap_out )
{
    ...
    wadfilenum = W_Load_WadFile( wadfilename ); // return filenum, or -1.
    if( wadfilenum == -1 )
    {
...
void  P_process_wadfile( int wadfilenum, /*OUT*/ level_id_t * firstmap_out )
{
    ...
    for (i=0; i<wadfile->numlumps; i++,lumpinfo++)
    {
        ...
        map_num = 0;  // invalid
        if (gamemode == GM_doom2_commercial)       // Doom2
        {
            if(    name[0]=='M'
                && name[1]=='A'
                && name[2]=='P' )
