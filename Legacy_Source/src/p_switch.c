...
static uint16_t *  switchlist_texnum = NULL;  // malloc
static unsigned int    max_numswitches = 0;  // allocated switch array
...
void P_Init_SwitchList(void)
{
  ...
  unsigned int   index = 0; // switchlist index
  unsigned int   i;
...
// __BIG_ENDIAN__ is defined on MAC compilers, not on WIN, nor LINUX
  ...
  if( switchlist_texnum == NULL )
      switchlist_texnum = malloc(sizeof(*switchlist_texnum));
  ...
  for (i=0;alphSwitchList[i].episode!=0;i++)
  {
    if (index+1 >= max_numswitches)  // use 2 at a time
    {
      ...
      switchlist_texnum = realloc(switchlist_texnum, sizeof(*switchlist_texnum) * (max_numswitches+1) );
    }
    ...
    if (alphSwitchList[i].episode <= episode)
    {
      ...
      if( tex1 == -1 )
      {
          tex1 = (i>1) ? switchlist_texnum[0] : 1;
          I_SoftError("Switch %i missing texture %s, using %i\n", i, alphSwitchList[i].name1, tex1 );
      }
      if( tex2 == -1 )
      {
          tex2 = (i>1) ? switchlist_texnum[1] : 1;
          I_SoftError("Switch %i missing texture %s, using %i\n", i, alphSwitchList[i].name2, tex2 );
      }
      switchlist_texnum[index++] = tex1;
      switchlist_texnum[index++] = tex2;
    }
  }
  ...
  if( index < max_numswitches )  // [WDJ] Prevent write outside of array
      switchlist_texnum[index] = 0xFFFF;  // unnecessary, never checked
                             ...
                             int           useAgain )
{
    ...
    sound = sfx_swtchn;
#if 0
    // [WDJ] This never worked before, and PrBoom and Heretic have it disabled.
    // To work, this must be before special = 0.
    // EXIT SWITCH?
    if (line->special == 11)
        sound = sfx_swtchx;
#endif
    ...
    {
        uint16_t sw_texnum = switchlist_texnum[i];
        if( sw_texnum == texTop )
        {
            S_StartXYZSound(buttonlist->soundorg, sound);
            swside->toptexture = switchlist_texnum[i^1];
            if (useAgain)
                P_StartButton(line, B_top_texture, sw_texnum, BUTTONTIME);
        ...
        {
            S_StartXYZSound(buttonlist->soundorg, sound);
            swside->midtexture = switchlist_texnum[i^1];
            if (useAgain)
                P_StartButton(line, B_middle_texture, sw_texnum, BUTTONTIME);
        ...
        {
            S_StartXYZSound(buttonlist->soundorg, sound);
            swside->bottomtexture = switchlist_texnum[i^1];
            if (useAgain)
                P_StartButton(line, B_bottom_texture, sw_texnum, BUTTONTIME);
                           ...
                           int           side )
{
    ...
    if (EN_boom)
    {
      ...
      else if (line_special >= GenFloorBase)
      {
        ...
        if (!line->tag && ((line_special&6)!=6)) //all non-manual
      ...
      {
        if (!thing->player)
          if (!(line_special & CrusherMonster))
            goto nopass;  // monsters disallowed
        if (!line->tag && ((line_special&6)!=6)) //all non-manual
          goto nopass;  //generalized types require tag
    ...
    switch (line_special)
    {
      ...
      case 107:
        if( EN_heretic )
        {
            if (EV_BuildStairs (line, FIXINT(16)))
                goto switch_1_clear;
        }
        break;
