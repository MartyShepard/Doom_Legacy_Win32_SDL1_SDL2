...
#endif
#ifdef NEW_CONFIG_NOTIFY
byte   new_config_notify_state = 1;
#endif
    ...
    for (i=0; i<currentMenu->numitems; i++)
    {
        ...
        switch (mip->status & IT_DISPLAY)
        {
           ...
           case IT_WHITESTRING :
               ...
               {
                   // Skip non-configurable in menu_cfg page.
               ...
               switch (mip->status & IT_TYPE)
               {
                 case IT_CVAR:
                 {
                   ...
                   {
                       // Skip when not in current menu_cfg page.
...
void M_StartServer( int choice )
{
    ...
    COM_BufAddText(va("map \"%s\" -skill %d -monsters %d\n", 
                      (gamemode == GM_doom2_commercial)? cv_nextmap.string : cv_nextepmap.string,
...
void M_StartServerMenu(int choice)
{
    ...
    ServerMenu[0] = (gamemode == GM_doom2_commercial)?
...
void M_Episode(int choice)
{
    ...
    // Yet another hack...
    if ( (gamemode == GM_doom_registered)
         && (choice > 2))
    {
...
void M_SingleNewGame(int choice)
{
    ...
    if ( gamemode == GM_doom2_commercial
         || (gamemode == GM_chexquest1 && !modifiedgame) //DarkWolf95: Support for Chex Quest
         )
        Push_Setup_Menu(&NewDef);
    else
...
void M_DrawVideoMode(void)
{
    ...
    if( menu_cfg )
    {
        // Video mode into current Menu_cfg page, editing.
        V_DrawString( 2, 1, V_WHITEMAP, menu_cfg_string[menu_cfg]);
        V_DrawString( BASEVIDWIDTH - (14*8), 1, V_WHITEMAP, "Insert Delete");
...
#ifdef CONFIG_MENU_PAGE
            // Detect current video settings from possible Menu_cfg dependent config.
...
#define msgtext  msgline.text
    ...
    // [WDJ] Tolerate extra wide maxlen.
    int message_width = (maxlen * 8) + 16;
    MessageDef.x= (message_width < BASEVIDWIDTH)? (BASEVIDWIDTH - message_width)/2 : 0;
...
{
#ifdef CONFIG_MENU_PAGE
    // Open cvar for editing, such as Menu_cfg editing.
...
void M_new_config_notify( void )
{
    ...
    // [WDJ] This might be saved in config someday.
    if( new_config_notify_state++ > 3 )
    {
        new_config_notify_state = 0;  // off
        return;
    }

    for( cv = CV_IteratorFirst(); cv ; cv = CV_Iterator( cv ) )
    {
        if( cv->flags & CV_HIDEN )  // skip these
            continue;

        if( (cv->flags & CV_SAVE) == 0 )  // require save
            continue;
        ...
        if( (cv->state & CS_CONFIG) == CFG_none )
        {
            ...
            if( bufcnt == 0 )
            {
                 bufcnt = snprintf( msgtmp, MSGTMP_LEN, "OPTIONS being defaulted. Use OPTIONS menu.\n" );
            }

            if( (ny > 16)  ||  (bufcnt > 220) )
            {
                snprintf( &msgtmp[bufcnt], MSGTMP_LEN - bufcnt, "Etc.\n" );
                break;
            }

            bufcnt += snprintf( &msgtmp[bufcnt], MSGTMP_LEN - bufcnt, "%s\n", cv->name );
            ny++;
        }
    }
    ...
    if( count )
    {
        M_StartMessage( msgtmp, NULL, MM_NOTHING );  // Display the new config vars
    }
    else
    {
        new_config_notify_state = 0;  // off
...
#ifdef NEW_CONFIG_NOTIFY_XXX
    if( new_config_notify_state )
    {
        M_new_config_notify();
        if( new_config_notify_state )
            return;
...
void M_Configure (void)
{
    ...
    cval = ( gamemode == GM_heretic )? (IT_CONTROL) : IT_LITLSPACE;
    for( i=0; i<ControlDef2.numitems; i++)
    {
