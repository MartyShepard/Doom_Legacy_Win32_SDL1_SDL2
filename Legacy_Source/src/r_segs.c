...
static void R_DrawSplatColumn (column_t* column)
{
    ...
    for ( ; column->topdelta != 0xff ; )
    {
        ...
        dc_yl = FIXED_TO_INT(top_post_sc + FRACUNIT-1);  // round up
...
//    seg = ds_p->curline;
    ...
    for ( ; splat ; splat=splat->next)
    {
        ...
        for (dc_x = x1 ; dc_x <= x2 ; dc_x++, dm_yscale += rw_scalestep)
        {
            ...
            fixed_t texturecolumn = rw_offset2 - splat->offset - FixedMul(finetangent[angf],rw_distance);
...
//                         dc_x,FIXED_TO_FLOAT(rw_offset2),FIXED_TO_FLOAT(splat->offset),angf,FIXED_TO_FLOAT(finetangent[angf]),FIXED_TO_FLOAT(rw_distance),FIXED_TO_FLOAT(FixedMul(finetangent[angf],rw_distance)));
            int texcol = FIXED_TO_INT( texturecolumn );
            if( texcol < 0 || texcol >= patch->width )
                continue;

            // draw the texture
            col = (column_t *) ((byte *)patch + patch->columnofs[texcol]);
...
void R_Render_PictureColumn ( byte * column_data )
{
    ...
    dc_yl = FIXED_TO_INT(dm_top_patch + FRACUNIT-1);  // round up
    dc_yh = FIXED_TO_INT(bottom_post_sc - 1);

    if(dm_windowtop != FIXED_MAX && dm_windowbottom != FIXED_MAX)
    {
      dc_yl = FIXED_TO_INT(dm_windowtop + FRACUNIT);
...
void R_RenderMaskedSegRange( drawseg_t* ds, int x1, int x2 )
{
    ...
    dc_texheight = FIXED_TO_INT(texheightz);
      ...
      if(maskedtexturecol[dc_x] != SHRT_MAX)
      {
        ...
        if(dc_num_lights)
        {
          ...
          for( gi = 0; gi < dc_num_lights; gi++)
          {
            ...
            {
              rlight->botheight += rlight->botheightstep;
              bheight = rlight->botheight - (FRACUNIT >> 1);  // - 1/2
            }
...
//    fixed_t  topheight, heightstep, sec_ceilingheight_viewrel;  // became unused
    ...
    {
        // random fog scale
        dm_yscale = ((int)fog_wave2 << (FRACBITS-6)) + FIXINT(14);  // 30 .. 14
        rw_scalestep = ((int)fog_wave1 << (FRACBITS-7)) - FIXINT(4);  // ( 4 .. -4 )
...
//              int bottom_w = ffp->front_clip_bot[rw_x] - 1;  // get black lines
        ...
        fixed_t texturecolumn = rw_offset - FixedMul(finetangent[angf], rw_distance);
        int texcol = FIXED_TO_INT( texturecolumn );
        ...
        if (midtexture)
        {
          if( yl < rdraw_viewheight && yh >= 0 && yh >= yl ) // not disabled
          {
            ...
            dc_texheight = FIXED_TO_INT( textureheight[midtexture] );
            //profile stuff ---------------------------------------------------------
#ifdef TIMING
            ProfZeroTimer();
#endif
            ...
            // Does not know if TM_picture or TM_patch	     
            R_Draw_WallColumn(mid_texren, texcol);
            ...
            if (toptexture)
            {
                ...
                if (mid >= yl)
                {
                    if( yl < rdraw_viewheight && mid >= 0 ) // not disabled
                    {
                        ...
                        dc_texheight = FIXED_TO_INT( textureheight[toptexture] );
                        // Does not know if TM_picture or TM_patch	     
                        R_Draw_WallColumn(top_texren, texcol);
                    } // if mid >= 0
            ...
            if (bottomtexture)
            {
                ...
                if (mid <= yh)
                {
                    if( mid < rdraw_viewheight && yh >= 0 ) // not disabled
                    {
                        ...
                        dc_texheight = FIXED_TO_INT( textureheight[bottomtexture] );

                        // Does not know if TM_picture or TM_patch	     
                        R_Draw_WallColumn(bottom_texren, texcol);
                    } // if mid >= 0
