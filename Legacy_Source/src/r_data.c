...
unsigned int  flatmemory;
unsigned int  spritememory;
unsigned int  texturememory;	// all textures
...
void  R_Set_Texture_Patch( uint16_t texnum, patch_t * patch )
{
...
byte* R_GenerateTexture2 ( uint16_t texnum, texture_render_t *  texren, byte  texture_req )
{
...
#define FIRST_TEXTURE  0
...
#endif
    {
	    ...
	    texture->detect |= TD_odd_width;
            k = 1;  // make width_tile_mask = 0
	}
        texture_render[i].width_tile_mask = k-1;
...
void R_PrecacheLevel (void)
{
    ...
    for (i=0 ; i<numsprites ; i++)
    {
        ...
        for (j=0 ; j<sprites[i].numframes ; j++)
        {
            ...
            for (k=0 ; k<n ; k++)
            {
		sprite_frot_t * sv = get_framerotation( &sprites[i], j, k );
                //Fab: see R_Init_Sprites for more about pat_lumpnum,lumpid
		lumpnum_t  lumpnum = sv->pat_lumpnum;
                if(devparm)
