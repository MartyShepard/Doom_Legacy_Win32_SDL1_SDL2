...
#ifdef WIN32
  #ifdef DRAGFILE
    static int isFullFilePath(const char *str);
    static int isWadFile(const char *path);
    static byte Get_DragFile(char *dragged_file);
  #endif
#endif
...
#else 
  static void  Console_Request(byte console_request);
  void  static Logmesg_Request(byte LogFile_Request);
  #ifdef SDL2
...
# endif
  ...
  #if defined(LOGMESSAGES) && defined(WIN32) 
