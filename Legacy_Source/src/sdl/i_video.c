...
// Marty: wäre schön wenn die so einfach laufen würden.Gell!?
...
*/
     ...
     * Borderless wird direkt bei der Wahl des Modus geändert und nicht via Flags wie bei SDL2
     */
...
int I_RequestFullGraphics( byte select_fullscreen )
{
    ...
    switch(req_drawmode)
    {
     ...
     case DRM_opengl:
       ...
       select_bytepp = native_bytepp;
       goto get_modelist;
     /* Marty: Ich habe die hinzugefügt, für mehr klarheit */
     case DRM_8pal:
 ...
 * 32Bit Desktop zur verfügung steht. Wobei mein Desktop
 * nur 8,16 und 32 bietet. läuft es es auch mit 15Bit
