...
boolean P_SightPathTraverse (fixed_t x1, fixed_t y1, fixed_t x2, fixed_t y2)
{
    ...
    {
        mapxstep = 1;
        partial = FRACUNIT - FRACTION_PART(x1>>MAPBTOFRAC);
        ystep = FixedDiv (y2-y1,abs(x2-x1));
    }
    else if (xt2 < xt1)
    {
        mapxstep = -1;
        partial = FRACTION_PART(x1>>MAPBTOFRAC);
        ystep = FixedDiv (y2-y1,abs(x2-x1));
    }
    else
    {
    ...
    {
        mapystep = 1;
        partial = FRACUNIT - FRACTION_PART(y1>>MAPBTOFRAC);
        xstep = FixedDiv (x2-x1,abs(y2-y1));
    }
    else if (yt2 < yt1)
    {
        mapystep = -1;
        partial = FRACTION_PART(y1>>MAPBTOFRAC);
        xstep = FixedDiv (x2-x1,abs(y2-y1));
    }
    else
    {
