...
fixed_t jumpgravity = (FIXINT(6)/NEWTICRATERATIO); // variable by fragglescript
...
void P_CalcHeight (player_t* player)
{
    ...
    if (player->cheats & CF_NOMOMENTUM)  // as in heretic because of fly bob
    {
        ...
        //player->viewz = pmo->z + INT_TO_FIXED(((unsigned int)cv_viewheight.EV));

        //if (player->viewz > pmo->ceilingz - FIXINT(4))
        //    player->viewz = pmo->ceilingz - FIXINT(4);
    ...
    // move viewheight
    calc_viewheight = INT_TO_FIXED((unsigned int)cv_viewheight.EV); // default eye view height
    ...
    if(player->chickenTics)  // heretic chicken morph
        player->viewz = smo->z + player->viewheight - FIXINT(20);
    else
    ...
    if (player->viewz > smo->ceilingz - FIXINT(4))
        player->viewz = smo->ceilingz - FIXINT(4);
    if (player->viewz < smo->floorz + FIXINT(4))
        player->viewz = smo->floorz + FIXINT(4);
}
...
#define  TIRED_THRESHOLD   (TICRATE*4)
        if( cv_tired_run.EV )
        {
            if( player->tired > TIRED_THRESHOLD )
            {
# if NEWTICRATERATIO > 1
                int tire_ex = (player->tired - TIRED_THRESHOLD) / (2 * NEWTICRATERATIO);
# else
                int tire_ex = (player->tired - TIRED_THRESHOLD) >> 1;
# endif
                int tf = (1024 - (tire_ex * cv_tired_run.EV));
                if( tf < 2 )  tf = 2;
                movefactor = (movefactor * tf) >> 10;
                bobfactor = (bobfactor * tf) >> 10;
            }

            if( player->tired < (TIRED_THRESHOLD + (TICRATE*45)))  // add to tired
            {
                player->tired += ( abs(cmd->forwardmove) + abs(cmd->sidemove) ) / (24/NEWTICRATERATIO);
            }
        }
#endif
    ...
    if (cmd->buttons & BT_JUMP)
    {
        ...
        if( onground && !(player->GB_flags & GB_jumpdown))
        {
...
/* HERETODO
    ...
    if(pmo->flags2&MF2_FLY)
    {
        pmo->momz = INT_TO_FIXED(player->flyheight);
        if(player->flyheight)
            player->flyheight /= 2;
    }
...
# ifdef TICCMD_148
    if( cmd->ticflags & TC_comehere )
    {
        come_here = 3*TICRATE;
        come_here_player = pmo;
        GenPrintf( EMSG_info, "Come here\n" );
#  ifdef DEBUG_COME_HERE	
        GenPrintf( EMSG_debug, "Come here ## \n" );
#  endif
    }
# endif
#endif
...
void P_DeathThink (player_t* player)
{
    ...
    // fall to the ground
    if (player->viewheight > FIXINT(6))
        player->viewheight -= FIXINT(1);

    if (player->viewheight < FIXINT(6))
        player->viewheight = FIXINT(6);
...
//#define VIEWCAM_DIST    FIXINT(128)
//#define VIEWCAM_HEIGHT  FIXINT(20)
...
void P_ResetCamera( player_t* player )
{
    ...
    x = player->mo->x;
    y = player->mo->y;
    z = player->mo->z + INT_TO_FIXED((unsigned int)cv_viewheight.EV);
...
void P_MoveChaseCamera (player_t * player)
{
    ...
    z = pmo->z + INT_TO_FIXED((unsigned int)cv_viewheight.EV) + cv_cam_height.value;
...
/*    P_PathTraverse ( pmo->x, pmo->y, x, y, PT_ADDLINES, PTR_UseTraverse );*/
    ...
    if (!newsubsec)
    {
        ...
        if (pmo->subsector->sector->ceilingheight - camera.mo->height < z)
            z = pmo->subsector->sector->ceilingheight - camera.mo->height - FIXINT(11); // don't be blocked by a opened door
    }
    else
    // camera fit ?
    ...
    {
        // no fit
        z = newsubsec->sector->ceilingheight - camera.mo->height - FIXINT(11);
    }
    ...
    dist=FLOAT_TO_FIXED(sqrt(f1*f1+f2*f2));
...
void P_PlayerThink (player_t* player)
{
    ...
    if( cv_drown.EV )
    {
        ...
        extern byte  checkwater_is_drowning;

        if( checkwater_is_drowning )  // from P_MobjCheckWater
        {
            // [WDJ] Effect of drown is to be tired, and darkening view.
            // We do not have a specialized death for this, and it is not worth it to make one.
            // Also, many maps will have misused the water, so minimize interact with that.
            if( player->drown > (TICRATE*55) )  // 55 sec
            {
                if( player->tired < 550 )  player->tired += 2;  // override tired--
            }

            if( player->drown < (TICRATE*300) )  player->drown ++;  // 300 sec
        }
        else if( player->drown )
        {
            // got air, recover
            if( player->drown > (TICRATE*60) )  // 60 sec
                player->drown -= 7;
            else
                player->drown --;
        }
...
# if 0
    if( player->tired | player->drown )  // DEBUG
        printf( "TIRED=%i  DROWN=%i\n", player->tired, player->drown );
# endif
#endif
    ...
    {
        if ((   pmo->momx >  FIXINT(2)
             || pmo->momx < -FIXINT(2)
             || pmo->momy >  FIXINT(2)
             || pmo->momy < -FIXINT(2)
             || pmo->momz >  FIXINT(2)   ) // jump out of water
...
void P_ArtiTele( player_t* player )
{
    ...
    destX = INT_TO_FIXED(mtp->x);
    destY = INT_TO_FIXED(mtp->y);
    destAngle = wad_to_angle(mtp->angle);
    P_Teleport_with_effects(player->mo, destX, destY, destAngle);
...
boolean P_UseArtifact( player_t* player, artitype_t arti )
{
    ...
    case arti_firebomb:
        ...
        mo = P_SpawnMobj(pmo->x + 24*finecosine[angle],
                         pmo->y + 24*finesine[angle],
                         pmo->z - (FIXINT(15) * ((pmo->flags2&MF2_FEETARECLIPPED) != 0)),
                         MT_FIREBOMB);
        mo->target = pmo;
        break;
    case arti_egg:
