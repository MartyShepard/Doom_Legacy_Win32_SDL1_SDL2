...
boolean  bot_tele_dest_found = false;
int  bot_tele_destx, bot_tele_desty;
uint16_t  bot_tele_spec;
int  xOffset, yOffset;
int  xSize, ySize;
unsigned int  numbotnodes;
        ...
        switch(line->special)
        {
            ...
            for (i = -1; (i = P_FindSectorFromLineTag(line, i)) >= 0;)
            {
                for (th = thinkercap.next; th != &thinkercap; th = th->next)
                {
                      ...
                      )
                    {
                        ...
                        bot_tele_destx = m->x;
                        bot_tele_desty = m->y;
                        bot_tele_spec = line->special;

                        //debug_Printf("found a teleport thing going to x:%d, y:%d\n", bot_tele_destx, bot_tele_desty);
                    }
                }
            }
            break;
          ...
          case 263:  //Same as 244 but reversed
            if( !bot_tele_dest_found )
            {
                for (i = -1; (i = P_FindLineFromLineTag(line, i)) >= 0;)
                {
                    ...
                    {
                        bot_tele_dest_found = true;
                        bot_tele_destx = (lines[i].v1->x+lines[i].v2->x)/2 - ((line->v1->x+line->v2->x)/2 - bot_tele_destx);
                        bot_tele_desty = (lines[i].v1->y+lines[i].v2->y)/2 - ((line->v1->y+line->v2->y)/2 - bot_tele_desty);
                        bot_tele_spec = line->special;
          ...
          default:	//not a special type
            ...
            if( s != oksector )
            {
                ...
                if( ((floorheight > (last_s->floorheight + FIXINT(45)))
                     || (((floorheight > (last_s->floorheight + FIXINT(37)))
...
boolean B_NodeReachable(mobj_t* mo, fixed_t x, fixed_t y, fixed_t destx, fixed_t desty)
{
    ...
    if ((nx >= 0) && (nx < xSize) && (ny >= 0) && (ny < ySize))
    {	
        bot_tele_destx = destx;
        bot_tele_desty = desty;
...
{
    fixed_t  x = x2ClosestPosX(bot_tele_destx);
    fixed_t  y = y2ClosestPosY(bot_tele_desty);

    //debug_Printf("trying to make a tele node at x:%d, y:%d\n", FIXED_TO_INT(bot_tele_destx), FIXED_TO_INT(bot_tele_desty));
        ...
        for (angle = BDI_EAST; angle <= BDI_SOUTHEAST; angle++)
        {
            ...
            switch(angle)
            {
              ...
              case (BDI_NORTHEAST):
                nx = node->x + 1;
                ny = node->y + 1;
                extraCost = 5000;	//because diagonal
                break;
              case (BDI_NORTH):
                nx = node->x;
                ny = node->y + 1;
...
//	yMin-=BOTNODEGRIDSIZE; yMax+=2*BOTNODEGRIDSIZE;
    ...
    for (i = 0; i < MAXPLAYERS; i++)
    {
        if (playerstarts[i])
        {
            px = playerstarts[i]->x / FIXED_TO_INT(BOTNODEGRIDSIZE) - xOffset;
            py = playerstarts[i]->y / FIXED_TO_INT(BOTNODEGRIDSIZE) - yOffset;
    ...
    for (i = 0; i < MAX_DM_STARTS; i++)
    {
        ...
        {
            px = deathmatchstarts[i]->x / FIXED_TO_INT(BOTNODEGRIDSIZE) - xOffset;
            py = deathmatchstarts[i]->y / FIXED_TO_INT(BOTNODEGRIDSIZE) - yOffset;
            if (((px >= 0) && (px < xSize) && (py >= 0) && (py < ySize)) && (!botNodeArray[px][py]))
            {
                tempNode = B_CreateNode(px, py);
                numbotnodes++;
